...
class ZoneDataParser {
  ...
  static ZoneParsingResult parseRawData(String rawData) {
    try {
      debugPrint('🔍 $_tag: Starting to parse zone data: ${rawData.length} chars');
      ...
      if (checksum == null) {
        debugPrint('❌ $_tag: Checksum extraction failed');
      ...
      if (deviceData.isEmpty) {
        debugPrint('❌ $_tag: No device data found');
        return _createErrorResult('NO_DEVICE_DATA', 'No device data found');
      }

      // Step 4: Validate checksum
      final calculatedChecksum = _calculateChecksum(deviceData);
      ...
      debugPrint('✅ $_tag: Checksum validated: $checksum');
      ...
      debugPrint('✅ $_tag: Parsing successful: ${result.cycleType}, ${result.status}, ${result.deviceCount} devices');
      return result;
  ...
  static String? _extractChecksum(String rawData) {
    try {
      ...
      debugPrint('❌ $_tag: Checksum extraction error: $e');
      return null;
    }
  }
  ...
  static String _extractDeviceData(String rawData) {
    try {
      ...
      debugPrint('❌ $_tag: Device data extraction error: $e');
      return '';
    }
  }

  /// Calculate checksum from device data
  static String _calculateChecksum(String deviceData) {
    try {
      int checksum = 0;

      // Simple XOR checksum calculation
      for (int i = 0; i < deviceData.length; i++) {
        checksum ^= deviceData.codeUnitAt(i);
      }

      return checksum.toRadixString(16).toUpperCase().padLeft(4, '0');
    } catch (e) {
      debugPrint('❌ $_tag: Checksum calculation error: $e');
      return '0000';
    }
  }
  ...
  static List<ZoneDevice> _parseDevices(String deviceData) {
    ...
    debugPrint('📊 $_tag: Parsed ${devices.length} devices');
    return devices;
  }
  ...
  static ZoneDevice? _parseSingleDevice(String devicePart) {
    try {
      ...
      debugPrint('⚠️ $_tag: Unknown device format: $devicePart');
      return null;
    } catch (e) {
  ...
  static ZoneDevice _parseDeviceWithStatus(String address, String statusData) {
    try {
      ...
      debugPrint('❌ $_tag: Device status parsing error: $e');
      return ZoneDevice(
        address: address,
  ...
  static String _decodeStatus(String hexStatus) {
    try {
      ...
      debugPrint('❌ $_tag: Status decode error: $e');
  ...
  static String _determineSystemStatus(List<ZoneDevice> devices) {
    ...
    final hasAlarms = devices.any((d) => d.type == ZoneDeviceType.alarm);
    final hasTroubles = devices.any((d) => d.type == ZoneDeviceType.trouble);
    final allOnline = devices.every((d) => d.type == ZoneDeviceType.onlineNormal);

    if (hasAlarms) {
      return 'alarm_detected';
    } else if (hasTroubles) {
      return 'partial_trouble';
    } else if (allOnline) {
      return 'all_devices_online';
    } else {
      return 'system_stable_no_alarm';
    }
  }
